...
from typing import Any, Dict, List, Optional, Union, TypeVar, ForwardRef
...
RESERVED = set([
    ...
    "if", "import", "importstr", "importbin", "in", "local", 
...
class JsonnetGeneratorInterface(ABC):
    ...
    @abstractmethod
    def to_jsonnet(self, name: Optional[str] = None, **kwargs) -> Union[Optional[str], Dict[str, Any]]:
        """
...
class Block(JSONWizard, JsonnetGeneratorInterface):
    ...
    def _generate_new_fn(self, name: str, attributes_in_new: Dict[str, Attribute], 
                       attributes: Dict[str, Attribute], **kwargs) -> str:
        """
...
def generate_provider(provider: str, provider_schema: ProviderSchema, provider_source: str, 
                    provider_version: str, artifacts_dir: str = "/artifacts") -> None:
    """
    ...
    try:
        ...
        output = subprocess.check_output(
            ["xargs", "-0", "-P", "16", "-I", "{}", "jsonnetfmt", "-i", "--string-style", "d", "{}"], 
            stdin=ps.stdout
        )
        ps.wait()
