{
  local block = self,
  new(terraformName, name, queryStatement, workgroup):: (
    {
      jsonnetTfMetadata:: {
        terraform:: {
          name:: terraformName,
          object:: "aws_athena_prepared_statement",
          type:: "resource",
          attributes:: ["description", "id", "name", "query_statement", "region", "workgroup"],
        },
      },
    }
    + block.withName(name)
    + block.withQueryStatement(queryStatement)
    + block.withWorkgroup(workgroup)
  ),
