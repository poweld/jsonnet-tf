{
  local block = self,
  new(terraformName, eventCategories, name, snsTopicArn, sourceType):: (
    {
      jsonnetTfMetadata:: {
        terraform:: {
          name:: terraformName,
          object:: "aws_dms_event_subscription",
          type:: "resource",
          attributes:: ["arn", "enabled", "event_categories", "id", "name", "region", "sns_topic_arn", "source_ids", "source_type", "tags", "tags_all"],
        },
      },
    }
    + block.withEventCategories(eventCategories)
    + block.withName(name)
