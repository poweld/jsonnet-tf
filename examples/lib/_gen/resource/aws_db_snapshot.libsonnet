{
  local block = self,
  new(terraformName, dbInstanceIdentifier, dbSnapshotIdentifier):: (
    {
      jsonnetTfMetadata:: {
        terraform:: {
          name:: terraformName,
          ...
          type:: "resource",
          attributes:: ["allocated_storage", "availability_zone", "db_instance_identifier", "db_snapshot_arn", "db_snapshot_identifier", "encrypted", "engine", "engine_version", "id", "iops", "kms_key_id", "license_model", "option_group_name", "port", "region", "shared_accounts", "snapshot_type", "source_db_snapshot_identifier", "source_region", "status", "storage_type", "tags", "tags_all", "vpc_id"],
        },
      },
    }
    + block.withDbInstanceIdentifier(dbInstanceIdentifier)
    + block.withDbSnapshotIdentifier(dbSnapshotIdentifier)
  ),
