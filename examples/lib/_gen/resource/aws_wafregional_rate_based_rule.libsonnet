{
  local block = self,
  new(terraformName, metricName, name, rateKey, rateLimit):: (
    {
      jsonnetTfMetadata:: {
        terraform:: {
          name:: terraformName,
          object:: "aws_wafregional_rate_based_rule",
          type:: "resource",
          attributes:: ["arn", "id", "metric_name", "name", "rate_key", "rate_limit", "region", "tags", "tags_all"],
        },
      },
    }
    + block.withMetricName(metricName)
    + block.withName(name)
    + block.withRateKey(rateKey)
    + block.withRateLimit(rateLimit)
  ),
  withId(value):: (
