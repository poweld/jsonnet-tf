{
  ...
  new(terraformName):: (
    {
      jsonnetTfMetadata:: {
        terraform:: {
          ...
          object:: "aws_instance",
          type:: "resource",
          attributes:: ["ami", "arn", "associate_public_ip_address", "availability_zone", "disable_api_stop", "disable_api_termination", "ebs_optimized", "enable_primary_ipv6", "force_destroy", "get_password_data", "hibernation", "host_id", "host_resource_group_arn", "iam_instance_profile", "id", "instance_initiated_shutdown_behavior", "instance_lifecycle", "instance_state", "instance_type", "ipv6_address_count", "ipv6_addresses", "key_name", "monitoring", "outpost_arn", "password_data", "placement_group", "placement_group_id", "placement_partition_number", "primary_network_interface_id", "private_dns", "private_ip", "public_dns", "public_ip", "region", "secondary_private_ips", "security_groups", "source_dest_check", "spot_instance_request_id", "subnet_id", "tags", "tags_all", "tenancy", "user_data", "user_data_base64", "user_data_replace_on_change", "volume_tags", "vpc_security_group_ids"],
        },
      },
    }
  ),
  ...
  withPrimaryNetworkInterface(value):: (
    local converted = if std.isArray(value) then value else [value];
    {
      primary_network_interface: value,
    }
  ),
  ...
  withPrimaryNetworkInterfaceMixin(value):: (
    local converted = if std.isArray(value) then value else [value];
    {
      primary_network_interface+: converted,
    }
  ),
