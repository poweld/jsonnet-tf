{
  ...
  new(terraformName, allocatedStorage, bucket, dbInstanceType, name, organization, password, username, vpcSecurityGroupIds, vpcSubnetIds):: (
    {
      jsonnetTfMetadata:: {
        terraform:: {
          name:: terraformName,
          object:: "aws_timestreaminfluxdb_db_cluster",
          type:: "resource",
          attributes:: ["allocated_storage", "arn", "bucket", "db_instance_type", "db_parameter_group_identifier", "db_storage_type", "deployment_type", "endpoint", "failover_mode", "id", "influx_auth_parameters_secret_arn", "name", "network_type", "organization", "password", "port", "publicly_accessible", "reader_endpoint", "region", "tags", "tags_all", "username", "vpc_security_group_ids", "vpc_subnet_ids"],
        },
      },
    }
  ...
  "#withDbParameterGroupIdentifier":: "The ID of the DB parameter group to assign to your DB cluster.  \t\t\t\t\tDB parameter groups specify how the database is configured. For example, DB parameter groups  \t\t\t\t\tcan specify the limit for query concurrency.",
  ...
  "#withDeploymentType":: "Specifies the type of cluster to create.",
  ...
  "#withFailoverMode":: "Specifies the behavior of failure recovery when the primary node of the cluster \t\t\t\t\tfails.",
  withFailoverMode(value):: (
    local converted = value;
    assert std.isString(converted) : '"failover_mode" expected to be of type "string"';
    {
      failover_mode: converted,
    }
  ),
  "#withName":: "The name that uniquely identifies the DB cluster when interacting with the  \t\t\t\t\tAmazon Timestream for InfluxDB API and CLI commands. This name will also be a  \t\t\t\t\tprefix included in the endpoint. DB cluster names must be unique per customer  \t\t\t\t\tand per region.",
  withName(value):: (
    local converted = value;
  ...
  "#withNetworkType":: "Specifies whether the networkType of the Timestream for InfluxDB cluster is  \t\t\t\t\tIPV4, which can communicate over IPv4 protocol only, or DUAL, which can communicate  \t\t\t\t\tover both IPv4 and IPv6 protocols.",
  ...
  "#withPubliclyAccessible":: "Configures the Timestream for InfluxDB cluster with a public IP to facilitate access.",
  ...
  "#withVpcSecurityGroupIds":: "A list of VPC security group IDs to associate with the Timestream for InfluxDB cluster.",
  ...
  "#withVpcSecurityGroupIdsMixin":: "A list of VPC security group IDs to associate with the Timestream for InfluxDB cluster.",
  ...
  "#withVpcSubnetIds":: "A list of VPC subnet IDs to associate with the DB cluster. Provide at least  \t\t\t\t\ttwo VPC subnet IDs in different availability zones when deploying with a Multi-AZ standby.",
  ...
  "#withVpcSubnetIdsMixin":: "A list of VPC subnet IDs to associate with the DB cluster. Provide at least  \t\t\t\t\ttwo VPC subnet IDs in different availability zones when deploying with a Multi-AZ standby.",
